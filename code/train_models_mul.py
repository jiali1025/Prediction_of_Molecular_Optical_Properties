...
def data_cv ( nfold , type , data_path1 , data_path2 ) :
    train_x = [ ]
    test_x = [ ]
    train_y = [ ]
    test_y = [ ]
    ...
    for i in range ( nfold ) :
        train_data_ = pd.read_csv ( os.path.join ( data_path1 , type + '_train_' + str ( i ) + '.csv' ) )
        test_data_ = pd.read_csv ( os.path.join ( data_path1 , type + '_test_' + str ( i ) + '.csv' ) )
        train_data_des = pd.read_csv ( os.path.join ( data_path2 , 'descriptors' + '_train_' + str ( i ) + '.csv' ) )
        test_data_des = pd.read_csv ( os.path.join ( data_path2 , 'descriptors' + '_test_' + str ( i ) + '.csv' ) )

        columns_des = train_data_des.columns.tolist ( )
        train_x_des = train_data_des [ columns_des [ 1 : ] ]
        test_x_des = test_data_des [ columns_des [ 1 : ] ]

        train_data = pd.merge ( train_data_ , train_x_des , left_index = True , right_index = True , sort = False )
        test_data = pd.merge ( test_data_ , test_x_des , left_index = True , right_index = True , sort = False )
...
def lr_cv ( x_train10 , y_train10 , x_test10 , y_test10 , type , save_path , nfold ) :
    ...
    for i in range ( nfold ) :
        ...
        lr_fit = LogisticRegressionCV ( cv = 5 , solver = 'lbfgs' , max_iter = 1000 , multi_class = "ovr" ,
                                        n_jobs = -1 ).fit (
            x_train , y_train )
    ...
    score = np.concatenate ( (np.reshape ( score_train_ , (-1 , 1) ) , np.reshape ( score_test_ , (-1 , 1) )) ,
                             axis = 1 )
...
def knn_cv ( x_train10 , y_train10 , x_test10 , y_test10 , type , save_path , nfold ) :
    ...
    grid_params = { 'n_neighbors' : [ 3 , 5 , 7 , 9 , 11 , 13 , 15 , 17 , 19 ] , 'weights' : [ 'distance' ] ,
                    'metric' : [ 'euclidean' ] }
...
if __name__ == '__main__' :
    ...
    data_path1 = (os.path.join ( dataLocation , 'pca_data' ))
    data_path2 = (os.path.join ( dataLocation , 'scaler_data' ))

    resultSaveLocation = '../results/mul_result/'
    ...
    x_morg_val , y_morg_train_val , y_morg_val = data_cv ( nfold , 'morg' , data_path1 , data_path2 )

    x_rd_train10 , x_rd_test10 , y_rd_train10 , y_rd_test10 , x_rd_train_val ,\
    x_rd_val , y_rd_train_val , y_rd_val = data_cv ( nfold , 'rd' , data_path1 , data_path2 )

    x_AP_train10 , x_AP_test10 , y_AP_train10 , y_AP_test10 , x_AP_train_val ,\
    x_AP_val , y_AP_train_val , y_AP_val = data_cv ( nfold , 'AP' , data_path1 , data_path2 )

    x_torsion_train10 , x_torsion_test10 , y_torsion_train10 , y_torsion_test10 , x_torsion_train_val ,\
    x_torsion_val , y_torsion_train_val , y_torsion_val = data_cv ( nfold , 'torsion' , data_path1 , data_path2 )
    ...
    lr_cv ( x_torsion_train10 , y_torsion_train10 , x_torsion_test10 , y_torsion_test10 , 'torsion' , save_path ,
            nfold )
    ...
    knn_cv ( x_torsion_train10 , y_torsion_train10 , x_torsion_test10 , y_torsion_test10 , 'torsion' , save_path ,
             nfold )
    ...
    gb_cv ( x_torsion_train10 , y_torsion_train10 , x_torsion_test10 , y_torsion_test10 , 'torsion' , save_path ,
            nfold )
    ...
    rf_cv ( x_torsion_train10 , y_torsion_train10 , x_torsion_test10 , y_torsion_test10 , 'torsion' , save_path ,
            nfold )
    ...
    nn_cv ( x_torsion_train10 , y_torsion_train10 , x_torsion_test10 , y_torsion_test10 , 'torsion' , save_path ,
            nfold )
